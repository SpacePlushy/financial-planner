...
export const Summary: React.FC<SummaryProps> = ({ className }) => {
  ...
  const { isLoading: isExporting, withLoading } = useLoadingState({
    delay: 100,
    minDuration: 300,
  });
  ...
  }, [currentSchedule, scheduleService, withLoading]);
    ...
    return (
      <div className={`${styles.summary} ${className || ''}`}>
        <div className={styles.header}>
          <SkeletonLoader height={32} width={200} variant="text" />
          ...
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className={styles.metricCard}>
              ...
              <div style={{ marginTop: 8 }}>
                <SkeletonLoader height={36} width="80%" variant="text" />
              </div>
              <div style={{ marginTop: 12 }}>
                <SkeletonLoader height={16} width="100%" variant="text" />
                <div style={{ marginTop: 4 }}>
                  <SkeletonLoader height={16} width="70%" variant="text" />
                </div>
              </div>
            </div>
          ))}
  ...
  return (
    <LoadingOverlay isLoading={isExporting} message="Exporting..." blur>
      <div className={`${styles.summary} ${className || ''}`}>
        <div className={styles.header}>
            ...
            <div className={styles.metricDetails}>
              ...
              <span className={styles.metricSubValue}>
                {metrics.totalWorkDays > 0
                  ? formatCurrency(
                      metrics.totalEarnings / metrics.totalWorkDays
                    )
                  : formatCurrency(0)}
              </span>
            </div>
