...
export const OptimizationProgress: React.FC = () => {
  ...
  // Track if progress is minimized
  ...
  if (isMinimized && (isOptimizing || isCompleted)) {
    return (
      <div
        className={styles.progressMinimized}
        onClick={() => setIsMinimized(false)}
        role="button"
        tabIndex={0}
        aria-label="Expand optimization progress"
      >
  ...
  if (
    ...
    return (
      <div
        className={`${styles.progressOverlay} ${isCompleted ? styles.completed : ''}`}
      >
